...
2. Run `bun run tui`. On first run you’ll be asked for:
   - **OpenAI API key** (optional; for the journal agent).
...
2. Create two databases in Notion with these properties (see `src/adapters/outbound/notion/notion-schema.ts` for full purpose and types):
   ...
   - **TODOs DB:** Title (title), Category (select: Work | Health | Personal | Learning), Due Date (date), Notes (text), Priority (select: High | Medium | Low), Done (checkbox or Status/select). If Done is a Status property (e.g. Todo / In Progress / Done), the app auto-detects the options; you can override with `NOTION_TODOS_STATUS`, `NOTION_TODOS_DONE_VALUE`, and `NOTION_TODOS_OPEN_VALUE` in `.env` or `~/.pa/settings.json` (see `.env.example`).
