...
**When:** The user asks to change an existing task's priority, due date, title, category, or notes (e.g. "set task 1 to high priority", "change the first todo to Medium", "make task 2 due tomorrow").

**What to do:** Call **update_todo** with `id_or_index` (1-based index: "1" = first open task, "2" = second, etc.) and **only** the field(s) to change (e.g. `priority: "High"`, `due_date: "YYYY-MM-DD"`). Listing tasks with `list_todos` does **not** update anything—you must call **update_todo** to apply the change.
